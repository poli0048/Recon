//The MSA Paint Tool holds the state of and provides the draw calls for a tool used for
//editing the min safe altitude data layer
//Author: Bryan Poling
...
bool MSATool::Draw_Button(void) {
	bool wasToolActive = toolActive;
	ImDrawList * draw_list = ImGui::GetWindowDrawList();
	draw_list->ChannelsSplit(2);

	auto style = ImGui::GetStyle();
	ImGui::PushID("MSA Tool");
	...
	if (ImGui::InvisibleButton("MSAButton", (Eigen::Vector2d) (RectLRCorner - RectULCorner)))
	...
	if (ImGui::InvisibleButton("MSAArrow", (Eigen::Vector2d) (ArrowLRCorner - ArrowULCorner)) && (!ImGui::IsPopupOpen("Popup")))
		ImGui::OpenPopup("MSA DropDown");
	bg_color = ImGui::IsItemHovered() ? ImColor(style.Colors[ImGuiCol_ButtonHovered]) : ImColor(style.Colors[ImGuiCol_Button]);
...
void MSATool::Draw_DropDown(Eigen::Vector2d const & PopupULCorner, float PopupWidth) {
	auto style = ImGui::GetStyle();
	...
	if (ImGui::BeginPopup("MSA DropDown")) {
		m_popupOpen = true;
		...
		if (m_shape == 0) {
			...
			m_radius = std::max(std::min(m_radius, 60.0f), 0.1f);
			ImGui::PopItemWidth();
		}
		else if (m_shape == 1) {
			ImGui::Text("Width (m):");
			...
			m_lengthX = std::max(std::min(m_lengthX, 120.0f), 0.1f);
			ImGui::PopItemWidth();
			ImGui::Text("Length (m):");
			ImGui::PushItemWidth(ImGui::GetContentRegionAvail().x);
			ImGui::InputFloat("##LengthYInputFloat", &m_lengthY, 0.1f, 0.5f, "%.1f");
			m_lengthY = std::max(std::min(m_lengthY, 120.0f), 0.1f);
			ImGui::PopItemWidth();
		...
		if (m_EditMode == 0) {
			ImGui::Text("Min Safe Altitude (m):");
			ImGui::PushItemWidth(ImGui::GetContentRegionAvail().x);
			ImGui::InputFloat("##MSAInputFloat", &m_value, 0.25f, 4.0f, "%.1f");
			ImGui::PopItemWidth();
		}
		
		float popupWidth  = std::max(100.0f, ImGui::CalcTextSize("Min Safe Altitude (m):  ").x + 2.0f*style.ItemSpacing.x);
...
void MSATool::DrawTriangle(Eigen::Vector2d const & p_min, float scale, ImDrawList * DrawList) {
...
void MSATool::Draw_Overlay(Eigen::Vector2d const & CursorPos_NM, ImDrawList * DrawList, bool CursorInBounds) {
...
void MSATool::DrawTool(Eigen::Vector2d const & CursorPos_NM, ImDrawList * DrawList) {
	if (m_shape == 0) { //Circle
...
void MSATool::ExecuteEdit(Eigen::Vector2d const & CursorPos_NM) {
	if ((m_shape == 0) && (m_EditMode == 0))
		Maps::DataTileProvider::Instance()->Paint_Circle(CursorPos_NM, double(m_radius), Maps::DataLayer::MinSafeAltitude, double(m_value));
	else if ((m_shape == 0) && (m_EditMode == 1))
		Maps::DataTileProvider::Instance()->Erase_Circle(CursorPos_NM, double(m_radius), Maps::DataLayer::MinSafeAltitude);
	else if ((m_shape == 1) && (m_EditMode == 0))
		Maps::DataTileProvider::Instance()->Paint_Rect(CursorPos_NM, m_lengthX, m_lengthY, m_angleDeg, Maps::DataLayer::MinSafeAltitude, double(m_value));
	else if ((m_shape == 1) && (m_EditMode == 1))
		Maps::DataTileProvider::Instance()->Erase_Rect(CursorPos_NM, m_lengthX, m_lengthY, m_angleDeg, Maps::DataLayer::MinSafeAltitude);
}
