...
bool MSATool::Draw_Button(void) {
	...
	if (ImGui::InvisibleButton("MSAButton", RectLRCorner - RectULCorner))
	...
	if (ImGui::InvisibleButton("MSAArrow", ArrowLRCorner - ArrowULCorner) && (!ImGui::IsPopupOpen("Popup")))
...
void MSATool::Draw_DropDown(Math::Vector2 PopupULCorner, float PopupWidth) {
	auto style = ImGui::GetStyle();
...
void MSATool::DrawTriangle(Math::Vector2 p_min, float scale, ImDrawList * DrawList) {
