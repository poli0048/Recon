...
AvoidanceZonesTool::AvoidanceZonesTool() :
	m_popupDims(200,200),
	m_popupOpen(false),
	toolActive(false),
	m_shape(0),
...
bool AvoidanceZonesTool::Draw_Button(void) {
	bool wasToolActive = toolActive;
	ImDrawList * draw_list = ImGui::GetWindowDrawList();
	draw_list->ChannelsSplit(2);

	auto style = ImGui::GetStyle();
	ImGui::PushID("Avoidance Zones Tool");
	...
	if (ImGui::InvisibleButton("AvoidanceZonesButton", (Eigen::Vector2d) (RectLRCorner - RectULCorner)))
	...
	if (ImGui::InvisibleButton("AvoidanceZonesArrow", (Eigen::Vector2d) (ArrowLRCorner - ArrowULCorner)) && (!ImGui::IsPopupOpen("Popup")))
		ImGui::OpenPopup("Avoidance Zones DropDown");
	bg_color = ImGui::IsItemHovered() ? ImColor(style.Colors[ImGuiCol_ButtonHovered]) : ImColor(style.Colors[ImGuiCol_Button]);
...
void AvoidanceZonesTool::Draw_DropDown(Eigen::Vector2d const & PopupULCorner, float PopupWidth) {
	auto style = ImGui::GetStyle();
	...
	if (ImGui::BeginPopup("Avoidance Zones DropDown")) {
		m_popupOpen = true;
...
void AvoidanceZonesTool::DrawTriangle(Eigen::Vector2d const & p_min, float scale, ImDrawList * DrawList) {
...
void AvoidanceZonesTool::Draw_Overlay(Eigen::Vector2d const & CursorPos_NM, ImDrawList * DrawList, bool CursorInBounds) {
...
void AvoidanceZonesTool::DrawTool(Eigen::Vector2d const & CursorPos_NM, ImDrawList * DrawList) {
	if (m_shape == 0) { //Circle
		double radiusInPixels = MetersToPixels(m_radius, CursorPos_NM(1), MapWidget::Instance().zoom);
		Eigen::Vector2d cursorPos_ScreenSpace = MapWidget::Instance().NormalizedMercatorToScreenCoords(CursorPos_NM);
		
		DrawList->AddCircle(cursorPos_ScreenSpace, float(radiusInPixels)+1.0f, IM_COL32(255, 255, 255, 255), 30, 2.0f);
		DrawList->AddCircle(cursorPos_ScreenSpace, float(radiusInPixels)-1.0f, IM_COL32(0, 0, 0, 255), 30, 2.0f);
	}
	else { //Rectangle
...
void AvoidanceZonesTool::ExecuteEdit(Eigen::Vector2d const & CursorPos_NM) {
	if ((m_shape == 0) && (m_EditMode == 0))
		Maps::DataTileProvider::Instance()->Paint_Circle(CursorPos_NM, double(m_radius), Maps::DataLayer::AvoidanceZones, 1.0);
	else if ((m_shape == 0) && (m_EditMode == 1))
		Maps::DataTileProvider::Instance()->Erase_Circle(CursorPos_NM, double(m_radius), Maps::DataLayer::AvoidanceZones);
	else if ((m_shape == 1) && (m_EditMode == 0))
		Maps::DataTileProvider::Instance()->Paint_Rect(CursorPos_NM, m_lengthX, m_lengthY, m_angleDeg, Maps::DataLayer::AvoidanceZones, 1.0);
	else if ((m_shape == 1) && (m_EditMode == 1))
		Maps::DataTileProvider::Instance()->Erase_Rect(CursorPos_NM, m_lengthX, m_lengthY, m_angleDeg, Maps::DataLayer::AvoidanceZones);
}
