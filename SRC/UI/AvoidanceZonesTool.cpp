...
bool AvoidanceZonesTool::Draw_Button(void) {
	...
	if (ImGui::InvisibleButton("AvoidanceZonesButton", RectLRCorner - RectULCorner))
	...
	if (ImGui::InvisibleButton("AvoidanceZonesArrow", ArrowLRCorner - ArrowULCorner) && (!ImGui::IsPopupOpen("Popup")))
...
void AvoidanceZonesTool::Draw_DropDown(Math::Vector2 PopupULCorner, float PopupWidth) {
	auto style = ImGui::GetStyle();
...
void AvoidanceZonesTool::DrawTriangle(Math::Vector2 p_min, float scale, ImDrawList * DrawList) {
