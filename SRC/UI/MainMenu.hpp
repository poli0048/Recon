	...
	if (ImGui::BeginMenuBar()) {
		...
		if (ImGui::BeginMenu("Simulator")) {
			...
			if (MyGui::MenuItem(u8"\uf04b", labelMargin, "Elgin (1 Drone)")) {
				...
				double alt = 329.18;
				DroneInterface::DroneManager::Instance().ClearSimulatedDrones();
				auto dronePtr = DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation B1"s, Eigen::Vector3d(lat, lon, alt));
				if (dronePtr != nullptr)
					dronePtr->SetSourceVideoFile(GetSimVideoFilePath(DatasetFolder / "Dataset-6 (Elgin 7-27-2020)"));
				MapWidget::Instance().StartAnimation(lat - eps, lat + eps, lon - eps, lon + eps);
			}
			if (MyGui::MenuItem(u8"\uf04b", labelMargin, "Grand Meadow (1 Drone)")) {
				double lat = 43.622595*PI/180.0;
				double lon = -92.561887*PI/180.0;
				double alt = 421.54;
				DroneInterface::DroneManager::Instance().ClearSimulatedDrones();
				auto dronePtr = DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation C1"s, Eigen::Vector3d(lat, lon, alt));
				if (dronePtr != nullptr)
					dronePtr->SetSourceVideoFile(GetSimVideoFilePath(DatasetFolder / "Dataset-8 (Grand Meadow 7-8-2020)"));
				MapWidget::Instance().StartAnimation(lat - eps, lat + eps, lon - eps, lon + eps);
			}
			if (MyGui::MenuItem(u8"\uf04b", labelMargin, "Lamberton (1 Drone)")) {
				...
				double alt = 345.03;
				DroneInterface::DroneManager::Instance().ClearSimulatedDrones();
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation D1"s, Eigen::Vector3d(lat, lon, alt));
				MapWidget::Instance().StartAnimation(lat - eps, lat + eps, lon - eps, lon + eps);
			}
			if (MyGui::MenuItem(u8"\uf04b", labelMargin, "Lamberton (3 Drones - Config 1)")) {
				...
				double alt = 345.03;
				DroneInterface::DroneManager::Instance().ClearSimulatedDrones();
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation E1"s, Eigen::Vector3d(lat, lon, alt));
				
				lat = 44.236120*PI/180.0;
				lon = -95.308018*PI/180.0;
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation E2"s, Eigen::Vector3d(lat, lon, alt));
				
				lat = 44.236544*PI/180.0;
				lon = -95.307398*PI/180.0;
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation E3"s, Eigen::Vector3d(lat, lon, alt));
				MapWidget::Instance().StartAnimation(lat - eps, lat + eps, lon - eps, lon + eps);
			}
			if (MyGui::MenuItem(u8"\uf04b", labelMargin, "Lamberton (3 Drones - Config 2)")) {
				...
				double alt = 345.03;
				DroneInterface::DroneManager::Instance().ClearSimulatedDrones();
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation F1"s, Eigen::Vector3d(lat, lon, alt));
				
				lat = 44.239201*PI/180.0;
				lon = -95.304062*PI/180.0;
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation F2"s, Eigen::Vector3d(lat, lon, alt));
				
				lat = 44.236248*PI/180.0;
				lon = -95.307414*PI/180.0;
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation F3"s, Eigen::Vector3d(lat, lon, alt));
			...
			if (MyGui::MenuItem(u8"\uf04b", labelMargin, "Lamberton (4 Drones - Config 1)")) {
				...
				double alt = 345.03;
				DroneInterface::DroneManager::Instance().ClearSimulatedDrones();
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation G1"s, Eigen::Vector3d(lat, lon, alt));
				
				lat = 44.239201*PI/180.0;
				lon = -95.304062*PI/180.0;
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation G2"s, Eigen::Vector3d(lat, lon, alt));
				
				lat = 44.236248*PI/180.0;
				lon = -95.307414*PI/180.0;
				DroneInterface::DroneManager::Instance().AddSimulatedDrone("Simulation G3"s, Eigen::Vector3d(lat, lon, alt));
