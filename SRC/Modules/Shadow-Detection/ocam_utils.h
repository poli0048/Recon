...
inline int get_ocam_model(struct ocam_model * myocam_model, char const * filename) {
...
//Un-project a point in the image plane: Compute a unit bearing vector in the camera frame towards all points that project to it.
inline void cam2world(double point3D[3], double point2D[2], struct ocam_model const * myocam_model) {
	double const * pol = myocam_model->pol;
...
//Project a point in the camera frame to the image plane.
inline void world2cam(double point2D[2], double point3D[3], struct ocam_model const * myocam_model) {
	double const * invpol = myocam_model->invpol;
...
inline void create_perspective_undistortion_LUT(cv::Mat & mapx, cv::Mat & mapy, struct ocam_model const * ocam_model, double sf) {
	int width = mapx.cols;
