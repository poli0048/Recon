...
namespace DroneInterface {
	WaypointMission CreateSampleWaypointMission(int NumWaypoints, bool CurvedTrajectories, bool LandAtEnd, Eigen::Vector2d const & StartPos_LL, double HAG) {
		...
		//Set up ENU frame under first waypoint (at surface of reference ellipsoid)
		Eigen::Vector3d Pos_ECEF = LLA2ECEF(Eigen::Vector3d(StartPos_LL(0), StartPos_LL(1), 0.0));
		...
		mission.Waypoints.back().RelAltitude  = HAG;
		...
		for (int n = 0; n < NumWaypoints; n++) {
			...
			Eigen::Vector3d LastPosition_ECEF = LLA2ECEF(Eigen::Vector3d(lastWaypoint.Latitude, lastWaypoint.Longitude, 0.0)); //Projected to ref ellipsoid
